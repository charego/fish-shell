...
In contrast to ``grep``, ``string match`` defaults to glob-mode, while ``string replace`` defaults to literal matching. If set to regex-mode, they use PCRE regular expressions, which is comparable to ``grep -P``. ``string match`` defaults to printing just the match, which is like ``grep -o`` (use ``--entire`` to enable grep-like behavior).
